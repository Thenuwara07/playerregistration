...
const Players: React.FC = () => {
  ...
  const [closeClubFilterExact, setCloseClubFilterExact] =
    useState<string>("all");
  ...
  const [districts, clubTextOptions] = useMemo(() => {
    const ds = Array.from(
      new Set(playersWithAge.map((p) => p.district).filter(Boolean))
    );
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return playersWithAge.filter((p) => {
      ...
      const matchesOpenAssoc =
        openAssocFilter === "all" ||
        p.player?.openAssociation?.code === openAssocFilter;

      const matchesCloseAssoc =
        closeAssocFilter === "all" ||
        p.player?.closeAssociation?.code === closeAssocFilter;

      const matchesOpenClubExact =
        openClubFilterExact === "all" ||
        p.player?.openClub?.code === openClubFilterExact;

      const matchesCloseClubExact =
        closeClubFilterExact === "all" ||
        p.player?.closeClub?.code === closeClubFilterExact;
            ...
            <Card className="lg:sticky lg:top-24">
              ...
              <CardContent>
                ...
                <div className="space-y-3">
                  <Select
                    value={openAssocFilter}
                    onValueChange={setOpenAssocFilter}
                  >
                    <SelectTrigger>
                  ...
                  <Select
                    value={closeAssocFilter}
                    onValueChange={setCloseAssocFilter}
                  >
                    <SelectTrigger>
                  ...
                  <Select
                    value={openClubFilterExact}
                    onValueChange={setOpenClubFilterExact}
                  >
                    <SelectTrigger>
                  ...
                  <Select
                    value={closeClubFilterExact}
                    onValueChange={setCloseClubFilterExact}
                  >
                    <SelectTrigger>
