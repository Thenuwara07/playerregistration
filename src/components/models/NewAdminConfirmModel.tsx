...
interface NewAdminConfirmModelProps {
  open: boolean;
  data: NewAdminData;
  onConfirm: () => void;
  onCancel: () => void;
...
const ConfirmProfileModal: React.FC<NewAdminConfirmModelProps> = ({
  ...
  console.log("Confirm Modal Data:-----------------------------------", data);
    ...
    >
      ...
      >
        <div style={{ padding: "16px 20px", borderBottom: "1px solid #eee" }}>
          <h3 style={{ margin: 0 }}>Confirm New Admin details</h3>
          <p style={{ margin: "6px 0 0", color: "#666" }}>
            Please review the details and confirm to create account.
          </p>
        </div>

        <div style={{ padding: 20 }}>
          <Row label="Full Name" value={display(data.fullName)} />
          <Row label="First Name" value={display(data.firstName)} />
          <Row label="Last Name" value={display(data.lastName)} />
          <Row label="gender" value={display(data.gender)} />
          <Row label="Date of Birth" value={display(data.dateOfbirth)} />
          <Row label="District" value={display(data.district)} />
          <Row label="NIC Number" value={display(data.idCardNumber)} />
          <Row label="Email" value={display(data.email)} />
          <Row label="Contact Number" value={display(data.contactNumber)} />
        </div>
